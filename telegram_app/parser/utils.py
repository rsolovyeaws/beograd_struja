"""Utility functions for parsing addresses.

Includes functions to separate settlements, streets, and house numbers,
and to check if an address is within a specified range.
"""
...
def parse_street_and_numbers(streets_str: str) -> list:
    """Parse a string of streets and house numbers into a list of dictionaries.

    Args:
        streets_str (str): The string containing streets and house numbers.

    Returns:
        list: A list of dictionaries with keys 'settlement', 'street', and 'house_range'.
    ...
    for part in parts:
        ...
        if street_match:
            ...
            if remaining:
                result.append({"settlement": settlement, "street": street, "house_range": remaining})
        else:
            # Otherwise, it"s a house number or range
            result.append({"settlement": settlement, "street": street, "house_range": part})

    return result


def split_settlement_and_street(street_str: str) -> tuple:
...
def clean_address_parts(settlement: str, street: str) -> tuple:
...
def get_numeric_and_alpha_parts(house_number: str) -> tuple:
    """Split the house number into numeric and alphabetic parts.
...
def is_cyrillic_within_range(user_alpha: str, start_alpha: str, end_alpha: str) -> bool:
    """Compare Cyrillic alphabetic parts to determine if the user"s alphabetic part is within the given range.
...
def is_within_range(user_address: dict, scheduled_address: dict) -> bool:
    """Compare user-provided address with a scheduled address.
    ...
    if (
        user_address["area"] == scheduled_address["municipality"]
        and user_address["street"] == scheduled_address["street"]
    ):
        ...
        if "-" in scheduled_address["house_range"]:
            ...
            if start_numeric is not None and end_numeric is not None and user_numeric is not None:  # noqa: SIM102
...
def remove_duplicate_addresses(user_data: dict) -> dict:
    """Remove duplicate addresses from the user data.

    Args:
        user_data (dict): The user data containing addresses.

    Returns:
        dict: The user data with duplicate addresses removed.
    ...
    for address in user_data["addresses"]:
        ...
        # If the address has not been seen yet, add it to the list of unique addresses
